...
As soon as you merge an application repo pull request (PR) it's automatically
deployed to a QA environment where it's your responsibility to test it and
deploy it to production without delay. Leaving undeployed commits on QA can
slow down the next developer — who might be trying to deploy urgent changes —
as their commits can't be deployed without also deploying yours.
...
instances and replaces them with new ones. AWS may automatically replace an
...
deployments and autoscaling. Because app servers are multiple and disposable
any data that needs to persist must go in a backing service like a database and
not, for example, on the app server's local filesystem.
...
repo's GitHub Actions **Deploy** workflow is automatically triggered and:

1. Generates a version number from the date and SHA of the Git commit that is being deployed
2. Builds a Docker image from the app's `Dockerfile` and tags it with the version number
...
Beanstalk launches a new batch of VMs, deploys the new Docker image to each of
the new VMs, checks that the new VMs are healthy, attaches the new VMs to the
load balancer so that they start serving requests, and if all looks good
finally terminates the old VMs.  For a brief time VMs running the previous and
new versions of the application are both serving requests at once. The exact
details depend on the environment's configured [deployment policy](https://docs.aws.amazon.com/elasticbeanstalk/latest/dg/using-features.deploy-existing-version.html).

Each app has a **health check endpoint** that must return 200 OK before
Elastic Beanstalk will attach a new VM to the load balancer
(see [How do our `/_status` endpoints work?](https://stackoverflow.com/c/hypothesis/questions/456))
Before and after new VMs are attached to the load balancer Elastic Beanstalk
also uses various other signals (response times, HTTP status codes,
operating system load, CPU usage) to decide whether the VMs are healthy and
the deployment has succeeded. See
...
1. Go to the GitHub repo's **Actions** tab and then go to the **Deploy** workflow
   ...
   ![`deploy.yml` workflow runs](https://raw.githubusercontent.com/hypothesis/onboarding/add-deployment-docs/docs/images/deploy_workflow_runs.png)

   Click on your worfklow run to see a summary of its progress:

   ![`deploy.yml` workflow summary](https://raw.githubusercontent.com/hypothesis/onboarding/add-deployment-docs/docs/images/deploy_workflow_summary.png)

2. Your PR is on QA when the **QA / Deploy** job completes successfully:

   ![A completed QA deployment job](https://raw.githubusercontent.com/hypothesis/onboarding/add-deployment-docs/docs/images/completed_qa_deployment.png)

   You should now briefly test your PR on QA. The workflow run's summary page
   shows a handy link to the app's QA environment.
   ...
   safe to deploy.  Sometimes it's clear that a PR won't affect production: for
   example a documentation change or an upgrade of a dependency that's only
   used in tests or development. Other times you may need to ask the PR's
   developer on Slack whether their PR is safe to deploy, or test it yourself.
   ...
   **Bump zope-interface from 5.5.1 to 5.5.2**
   ...
   ![Undeployed PRs](https://raw.githubusercontent.com/hypothesis/onboarding/add-deployment-docs/docs/images/undeployed_prs.png)
   </details>
...
3. Once you're satisfied that it's working you should deploy your PR to production.
   ...
   ![Approving a deployment to production](https://raw.githubusercontent.com/hypothesis/onboarding/add-deployment-docs/docs/images/approving_a_deploy.png)

4. Your PR is on production when the **Production / Deploy** job completes successfully:

   ![A completed production deployment job](https://raw.githubusercontent.com/hypothesis/onboarding/add-deployment-docs/docs/images/completed_production_deployment.png)

   You should now briefly tests your PR on production and make sure that it's working.
...
![Deploying PRs in batch](https://raw.githubusercontent.com/hypothesis/onboarding/add-deployment-docs/docs/images/batch_deployment.png)
...
rebooting the app's servers without upgrading them to a newer version of the
code. This is often used during incidents or outages.
...
![Redeploying](https://raw.githubusercontent.com/hypothesis/onboarding/add-deployment-docs/docs/images/redeploying.png)
...
leave different environments or different app servers within the same
environment running different versions of the code, which could produce
inconsistent and unpredictable behavior. Undeployed commits on `main` can slow
down the next developer — who might be trying to deploy urgent changes — as
their commits can't be deployed without also deploying yours.
...
![Re-running a failed deployment](https://raw.githubusercontent.com/hypothesis/onboarding/add-deployment-docs/docs/images/failed_deployment.png)
...
If you rolled back the deployment to an older commit but the the problematic
commit remained the latest commit on the `main` branch then the bug will get
redeployed along with the next PR.

Instead, we always roll forward at a steady pace:
...
1. Send a PR with a new commit that fixes the issue or even simply reverts the
   ...
   ![Reverting a PR](https://raw.githubusercontent.com/hypothesis/onboarding/add-deployment-docs/docs/images/reverting.png)
